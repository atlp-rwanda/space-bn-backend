...
exports.sendFacilityFeedback = async (req, res) => {
  ...
  if (facilityNotFound == true) return res.status(404).json({ success: false, message: res.__('Facility not found') });
  const newFeedback = {
...
exports.getByFeedbackId = async (req, res) => {
  ...
  if (userNotFound) return res.status(400).json({ message: res.__('Feedback sender not found') });
...
exports.deleteFeedback = async (req, res) => {
  const currentUser = await decodeToken(req, res);
  const { feedback, feedbackNotFound } = await findFeedbackById(req.params.feedbackId);
  if (feedbackNotFound) return res.status(404).json({ message: res.__('Feedback not found') });
  if (currentUser.id != feedback.dataValues.senderId && currentUser.roleId != 3 && currentUser.roleId != 1) return res.status(400).json({ message: res.__('Access denied!') });
  try {
    await model.FacilitiesFeedback.destroy({
      where: { id: req.params.feedbackId }
    });
  } catch (e) {
    return res.status(500).send({ message: res.__('Error') });
  }
  return res.status(200).json({
    success: true,
...
exports.updateFeedbackStatus = async (req, res) => {
  ...
  if (currentUser.roleId != 3 && currentUser.roleId != 1) return res.status(401).json({ message: res.__('Access denied!') });
...
const markFeedbackSeen = async (res, feedbackId) => {
  await model.FacilitiesFeedback.update({ seen: true }, {
    where: {
      id: feedbackId
    }
  });
...
const unMarkFeedbackSeen = async (res, feedbackId) => {
  await model.FacilitiesFeedback.update({ seen: false }, {
    where: {
      id: feedbackId
    }
  });
...
exports.getFeedBacksByFacilityId = async (req, res) => {
  ...
  const feedbacks = await model.FacilitiesFeedback.findAll({ where: { facilityId } });
  return res.status(200).json({
    feedbacks
  });
};
...
const findFacilityById = async (facilityId) => {
  ...
  try {
    ...
    if (!facility) return { facilityNotFound: true };
    return { facility };
  } catch (e) {
...
const findFeedbackById = async (feedbackId) => {
  ...
  try {
    ...
    if (!feedback) return { feedbackNotFound: true };
    return { feedback };
  } catch (e) {
