...
const udpateProfile = async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ success: false, message: res.__('User not found') });
    if (user.dataValues.id != userId) return res.status(401).json({ success: false, message: res.__('Access denied. You can only update your profile') });
  } catch (e) {
  ...
  for (const prop in currentUser) {
    if (payloads.hasOwnProperty(prop)) {
      ...
      if (payloads[prop].save == true) capturedData.push(prop);
    }
  }
...
const getUserProfile = async (req, res) => {
  ...
  try {
    ...
    if (!profilePayloads.dataValues) return res.status(404).json({ success: false, message: res.__('Profile not found') });
    if (profilePayloads.dataValues.id != userId) return res.status(401).json({ success: false, message: res.__('Access denied. You can only update your profile') });
  } catch (e) {
    return res.status(404).json({ success: false, message: res.__('Profile not found') });
  }
  for (const prop in profilePayloads.dataValues) {
    const subObj = {};
    subObj.value = profilePayloads.dataValues[prop];
    profile[prop] = subObj;
    subObj.save = false;
    if (profilePayloads.dataValues.savedData) {
      const savedDataIndex = profilePayloads.dataValues.savedData.indexOf(prop);
      if (savedDataIndex != -1) {
        subObj.save = true;
      }
    }
